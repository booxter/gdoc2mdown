...
def parse_newsletter(text):
    ...
    for line in text.split('\n'):
        ...
        date_range_re = re.compile(
            r'^({0})\s+(\d+)\s+\-\s+({0})\s+(\d+)\s*$'.format(months_join))
        ...
        news_entry_re = re.compile(
            r'^({})\s+(\d+):\s+(.*)$'.format(months_join))
...
def format_newsletter(doc):
    text = header.format(doc['date_range'], today.strftime('%Y-%m-%d'))

    num = '{:03}'.format(doc['number'])
    text += (
        '\n{{{{< figure src="/img/{0}/{0}-1.jpeg" '
